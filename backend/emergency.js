      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;border:1px solid #eee;border-radius:12px;overflow:hidden;">
          ...
          <div style="padding:1.5rem;">
            ...
            <p style="color:#333;">Someone has requested your blood type. Please log in to accept or reject.</p>
            <table style="width:100%;border-collapse:collapse;margin-top:1rem;">
              ...
              <tr>
                ...
                <td style="padding:8px 12px;color:#e53935;font-weight:800;">${request.bloodRequired}</td>
              </tr>
              <tr style="background:#fff5f5;">
            ...
            <p style="margin-top:1.5rem;color:#888;font-size:0.88rem;">
              Log in to Health Tech Connect → Request page to accept this request.
            </p>
          </div>
        </div>
      `
...
// ── POST: Create a new blood request (targeted to one donor) ──
router.post('/', auth, async (req, res) => {
  try {
    ...
    // Send email only to the specific donor who was requested
    if (targetDonorId && targetDonorEmail) {
      const donor = await User.findById(targetDonorId).select('name');
      sendRequestEmailTodonor(request, targetDonorEmail, donor?.name || 'Donor');
    }
...
// Shows requests where targetDonor matches this user, status pending or accepted
...
// ── GET: Requests the logged-in user created ──
